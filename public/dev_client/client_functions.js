...
function createPlayerSprite(player_index) {
    ...
    if (current_view === 'galaxy') {
        ...
        } else if (objects[ship_index].object_type_id === 271) {                    // DESTROYER
        ...
        } else if (objects[ship_index].object_type_id === 345) {                    // GARDEN SHIP
            new_texture_key = 'player-garden-ship';
            new_texture_animation_key = 'player-garden-ship-animation';
        ...
        if (used_skin === false) {
            ...
            // ACID
            if (objects[body_index].object_type_id === 454) {
                new_texture_key = 'player-acid';
                new_texture_animation_key = 'player-acid-idle-animation';
            } 
            // HUMAN
            else if (objects[body_index].object_type_id === 110) {
                new_texture_key = 'player-human';
                new_texture_animation_key = 'player-human-idle-animation';
                new_sprite_y_offset = -6;
            }
            // INFERNO
            else if (objects[body_index].object_type_id === 453) {
                new_texture_key = 'player-inferno';
                new_texture_animation_key = 'player-inferno-idle-animation';
            }
            // MANUFACTURING
    ...
    // We get to create it
    if (!players[player_index].sprite) {
        console.log("Need to create sprite for player " + players[player_index].name);

        let player_info = getPlayerInfo(player_index);

        if (player_info.coord) {
            console.log("Player has coord");

            if(scene_game.textures.exists(new_texture_key)) {

                players[player_index].sprite = scene_game.add.sprite(-1000, -1000, new_texture_key);
                console.log("Created and added new sprite to player");
        ...
        // It's a ship sprite for another player. They are in their ship view, but we are in the galaxy view. Their ship is around us
            ...
            console.log("%c No idea where to put player sprite for player id: " + players[player_index].name + "!", log_warning);
...
function generateDiscoveryDisplay(object_type_id) {
    ...
    html_string += "</div></div>";
    sel_discoveries.append(html_string);
...
function generateFactionDisplay() {
    ...
    let player_faction_linker_index = faction_linkers.findIndex(function(obj) { return obj && obj.player_id === players[client_player_index].id; });
    ...
    let html_string = "";
    html_string += "<div class='message is-dark message-inline'><div class='message-body'><strong>Factions:</strong><br>";
...
function generatePlayerInfoDisplay() {
    ...
    if (!sel_player_stats.is(":visible")) {
        return false;
    }
    ...
    // lets start doing some client side level stuff just to see how it shows
    let manufacturing_level = getPlayerLevel('manufacturing');
    ...
    sel_player_stats.append(html_string);

}

function generateRelationshipDisplay() {

    if (!sel_relationships.is(":visible")) {
        return false;
    }

    sel_relationships.empty();
    ...
    for (let linker of player_relationship_linkers) {
        ...
        if (linker.race_id) {
            ...
            sel_relationships.append(adding_string);
        }
...
function generateResearchDisplay() {
    ...
    sel_research.append(adding_string);
}
...
function getRaceEatingLinkerIndex(race_id, object_type_id) {

    return race_eating_linkers.findIndex(function(obj) { return obj && obj.race_id === race_id &&
...
function printAssemblyList(assembling_object = false, coord = false) {
    ...
    // Object isn't already assembling something
    else {
        ...
        html_string += "<div class='message is-info'><div class='message-header'>Things You Can Assemble</div><div>";
            ...
            if (object_type.required_research_object_type_id) {
                ...
                if (player_research_linker_index !== -1) {
                    if (player_research_linkers[player_research_linker_index].researches_completed >= object_types[required_object_index].research_times_required) {
                        ...
                        research_requirement_met = false;
                        failure_reason = "<br>Research " + object_types[required_object_index].name;
                    }
                } else {
                    research_requirement_met = false;
                    failure_reason = "<br>Research " + object_types[required_object_index].name;
    ...
    for (let addiction_linker of active_addiction_linkers) {
        ...
        if (addiction_linker && addiction_linker.addicted_body_id === players[client_player_index].body_id) {
            ...
            adding_string += "<br>";
                ...
                console.log("%c Warning: Race doesn't have an eating linker for this!", log_warning);
            }
...
function generateAiManagementDisplay(object_index = -1) {
    ...
    if (!sel_ai_management.is(":visible")) {
        return false;
    }
    ...
    sel_ai_management.empty();
    sel_ai_management.append(html_string);
...
function generateAssemblyListItem(object_type, object_assembly_linkers, assembling_object = false, can_assemble = true, failure_reason = false) {
    ...
    let html_string = "";
    html_string += "<div class='assembly-item'";
    
    if(difficulty === "easy") {
        html_string += " style='box-shadow: 0px 0px 3px #315e1b; background-color: #f3ffed;' ";
    } else if(difficulty === "wasteful") {
        html_string += " style='box-shadow: 0px 0px 3px #8e8034; background-color: #fff8dd;' ";
    } else if(difficulty === "impossible" ) {
        html_string += " style='box-shadow: 0px 0px 3px #bf0000; background-color: #ffebeb;' ";
    }

    html_string += "><div><strong>" + object_type.name + "</strong>";

    if(object_type.complexity > manufacturing_level + 10) {
        can_assemble = false;
        html_string += " <span class='tag is-danger' style='padding:2px; line-height:1; height: 1.4em;'>Too Complex</span>";
    } else if(object_type.complexity > manufacturing_level) {
        html_string += " <span class='tag is-warning' style='padding:2px; line-height:1; height: 1.4em;'>Wasteful</span>";
    } else {
    ...
    if (failure_reason !== false) {
        ...
        object_assembly_linkers.forEach(function (assembly_linker) {
            ...
            let amount_needed = assembly_linker.amount;
            if(manufacturing_level < object_type.complexity && can_assemble) {
                let level_difference = object_type.complexity - manufacturing_level;
                amount_needed = assembly_linker.amount + Math.ceil((assembly_linker.amount / (10 - level_difference)) * 3);

                let difference_in_amount_needed = amount_needed - assembly_linker.amount;
                html_string += "<br> " + assembly_linker.amount + " +" + difference_in_amount_needed + " ";
            } else {
                html_string += "<br> " + amount_needed + " ";
...
function generateCanAssembleListDeprecated() {
    ...
    if (sel_can_assemble_list.is(':empty')) {
        sel_can_assemble_list.append("We have a <a target='_blank' href='https://space.alphacoders.com/site/tutorial'>tutorial</a> that can help you get started");
    }
...
function generateEquipmentDisplay() {
    ...
    if (!sel_player_equipment.is(":visible")) {
        return false;
    }
    ...
    equipment_linkers.forEach(function (equipment_linker) {
        if (equipment_linker.body_id === players[client_player_index].body_id) {
            ...
            if(object_types[object_type_index].manufacturing_modifier) {
                equipment_linker_string += " + " + object_types[object_type_index].manufacturing_modifier + " manufacturing";
            }
    ...
    sel_player_equipment.empty();
    sel_player_equipment.append(equipment_layout_string);
...
function generateManagementOptionsDisplay() {
    ...
    if (!sel_management_options.is(":visible")) {
        return false;
    }
    ...
    sel_management_options.empty();
    sel_management_options.append(html_string);
}
...
function generateAreaManagementDisplay() {
    ...
    sel_area_management.empty();
    sel_area_management.append(html_string);
}
...
function generateShipManagementDisplay() {
    ...
    if (!sel_ship_management.is(":visible")) {
        return false;
    }
    ...
    sel_ship_management.empty();
    sel_ship_management.append(html_string);
}
...
function getCoordNeighbor(coord_type, coord_index, direction) {
    ...
    if(direction === 'down') {
        ...
        if(debug_coord_ids.indexOf(current_coord.id) !== -1) {
            console.log("coord id: " + current_coord.id + " set down coord index to: " + current_coord.down_coord_index);
        }
        ...
        if(adjacent_coord_index !== -1 && coord_type === 'ship' && typeof ship_coords[adjacent_coord_index].up_coord_index === "undefined") {
            ...
            if(debug_coord_ids.indexOf(ship_coords[adjacent_coord_index].id) !== -1) {
                console.log("%c Updated coord id: " + ship_coords[adjacent_coord_index].id + " up_coord_index from: " + ship_coords[adjacent_coord_index].up_coord_index + " to: " + 
                    coord_index);
            }
            ship_coords[adjacent_coord_index].up_coord_index = coord_index;
        /*
        if(adjacent_coord_index !== -1 && coord_type === 'ship' && ( typeof ship_coords[adjacent_coord_index].up_coord_index === "undefined" || ship_coords[adjacent_coord_index].up_coord_index === -1 )) {
            ship_coords[adjacent_coord_index].up_coord_index = coord_index;

            console.log("Ship coord needed up coord index updated");

            if(ship_coords[adjacent_coord_index].object_type_id) {
                let adjacent_object_type_index = getObjectTypeIndex(ship_coords[adjacent_coord_index].object_type_id);
                if(object_types[adjacent_object_type_index].is_wall) {
                    console.log("Redrawing wall");
                    mapAddWall(ship_coords[adjacent_coord_index], adjacent_object_type_index, ship_coords[adjacent_coord_index].tile_x, ship_coords[adjacent_coord_index].tile_y);
                }
        ...
        if(adjacent_coord_index !== -1 && coord_type === 'ship' && typeof ship_coords[adjacent_coord_index].down_coord_index === "undefined") {
            ...
            if(debug_coord_ids.indexOf(ship_coords[adjacent_coord_index].id) !== -1) {
                console.log("%c Updated coord id: " + ship_coords[adjacent_coord_index].id + " down_coord_index from: " + ship_coords[adjacent_coord_index].down_coord_index + " to: " + 
                    coord_index);
            }
            ship_coords[adjacent_coord_index].down_coord_index = coord_index;

        /*
        if(adjacent_coord_index !== -1 && coord_type === 'ship' && (typeof ship_coords[adjacent_coord_index].down_coord_index === "undefined" || ship_coords[adjacent_coord_index].down_coord_index === -1)) {
            ship_coords[adjacent_coord_index].down_coord_index = coord_index;
            console.log("Ship coord needed down coord index updated");
            if(debug_coord_ids.indexOf(ship_coords[adjacent_coord_index].id) !== -1) {
                console.log("Our debug coord id " + ship_coords[adjacent_coord_index].id + " needed its down coord index updated");
            }
...
function mapAddObject(object) {
    ...
    if (display_linkers.length > 0) {
        ...
        // we can draw the object based directly on its linkers
        else {
            display_linkers.forEach(function (linker) {
                let linker_tile_x = object_info.coord.tile_x + linker.position_x;
                let linker_tile_y = object_info.coord.tile_y + linker.position_y;
...
function mapAddWall(coord, object_type_index, tile_x, tile_y) {
    ...
    if(coord.down_coord_index !== -1) {
        ...
        if(debug_coord_ids.indexOf(coord.id) !== -1) {
            console.log("There is a coord below. down_coord_index: " + coord.down_coord_index);
        }
    ...
    if(coord.up_coord_index !== -1) {
        ...
        if(notFalse(up_coord) && up_coord.object_type_id) {
            if(debug_coord_ids.indexOf(coord.id) !== -1) {
                console.log("Coord above has an object type id");
            }
            ...
            if(object_types[up_coord_object_type_index].is_wall) {
                if(debug_coord_ids.indexOf(coord.id) !== -1) {
                    console.log("Coord above object type is a wall");
                }
                wall_above = true;
            }

            if(debug_coord_ids.indexOf(coord.id) !== -1) {
                console.log("Coord above (id: " + up_coord.id + ") down_coord_index: " + up_coord.down_coord_index);
            }
    ...
    if(debug_coord_ids.indexOf(coord.id) !== -1) {
        console.log("At the end of mapAddWall, coord id: " + coord.id + " down_coord_index: " + coord.down_coord_index);
    }
...
monster_sprites.push({ 'key': 'alphablue',              'monster_type_id': 145, 'frame_width': 126, 'frame_height': 128, 'frame_count': 10, 'frame_rate': 18 });
monster_sprites.push({ 'key': 'beetle-knight',          'monster_type_id': 148, 'frame_width': 96, 'frame_height': 96, 'frame_count': 7, 'frame_rate': 8 });
...
monster_sprites.push({ 'key': 'bloomer',                'monster_type_id': 133, 'frame_width': 64, 'frame_height': 72, 'frame_count': 4, 'frame_rate': 8 });
monster_sprites.push({ 'key': 'blossomtis',             'monster_type_id': 116, 'planet_type_id': 29, 'frame_width': 68, 'frame_height': 68, 'frame_count': 6, 'frame_rate': 8 });
monster_sprites.push({ 'key': 'bluearray',              'monster_type_id': 143, 'frame_width': 64, 'frame_height': 64, 'frame_count': 10, 'frame_rate': 14 });
monster_sprites.push({ 'key': 'blueware',               'monster_type_id': 144, 'frame_width': 64, 'frame_height': 64, 'frame_count': 6, 'frame_rate': 14 });
...
monster_sprites.push({ 'key': 'cherree',                'monster_type_id': 117, 'planet_type_id': 29, 'frame_width': 144, 'frame_height': 132, 'frame_count': 7, 'frame_rate': 8 });
...
monster_sprites.push({ 'key': 'converted-mechanoid',    'monster_type_id': 146, 'frame_width': 72, 'frame_height': 70, 'frame_count': 18, 'frame_rate': 8 });
...
monster_sprites.push({ 'key': 'discarded-lancer',       'monster_type_id': 151, 'frame_width': 82, 'frame_height': 82, 'frame_count': 9, 'frame_rate': 8 });
monster_sprites.push({ 'key': 'forest-prince',          'monster_type_id': 147, 'frame_width': 160, 'frame_height': 140, 'frame_count': 8, 'frame_rate': 8 });
...
monster_sprites.push({ 'key': 'infantry',               'monster_type_id': 135, 'frame_width': 86, 'frame_height': 116, 'frame_count': 7, 'frame_rate': 8 });
...
monster_sprites.push({ 'key': 'mini-walker',            'monster_type_id': 149, 'frame_width': 82, 'frame_height': 88, 'frame_count': 10, 'frame_rate': 8 });
...
monster_sprites.push({ 'key': 'rottex',                 'monster_type_id': 91, 'frame_width': 96, 'frame_height': 96, 'frame_count': 7, 'frame_rate': 8 });
...
monster_sprites.push({ 'key': 'servant',                'monster_type_id': 152, 'frame_width': 110, 'frame_height': 106, 'frame_count': 8, 'frame_rate': 8 });
...
monster_sprites.push({ 'key': 'tentacles',              'monster_type_id': 159, 'frame_width': 64, 'frame_height': 64, 'frame_count': 3, 'frame_rate': 8 });
...
function loadPlayerSprites(object_type_id) {
    ...
    // ACID BODY
    if(object_type_id === 454) {

        let scene_game = game.scene.getScene('sceneGame');

        if(!scene_game.textures.exists('player-acid')) {
            scene_game.load.on('filecomplete', processPlayerFile, this);
            scene_game.load.spritesheet('player-acid', "https://space.alphacoders.com/images/acid-body-animated.png",
                { frameWidth: 64, frame_height: 66, endFrame: 30 });
            scene_game.load.start();
        }
    } 
    // DESTROYER
    if(object_type_id === 271) {
    ...
    // GARDEN SHIP
    else if(object_type_id === 345) {
        console.log("Loading in garden ship");
        let scene_game = game.scene.getScene('sceneGame');
    ...
    // INFERNO BODY
    else if(object_type_id === 453) {

        let scene_game = game.scene.getScene('sceneGame');

        if(!scene_game.textures.exists('player-inferno')) {
            scene_game.load.on('filecomplete', processPlayerFile, this);
            scene_game.load.spritesheet('player-inferno', "https://space.alphacoders.com/images/inferno-body-animated.png",
                { frameWidth: 64, frame_height: 68, endFrame: 30 });
            scene_game.load.start();
        }
    } 
    // RESEARCHING BODY
    else if(object_type_id === 411) {
...
function processPlayerFile(key, type, texture) {
    ...
    if(key === 'player-acid') {
        ...
        console.log("Processing acid body");
        loaded_object_type_id = 454;
        loaded_type = "body";


        let player_acid_idle_config = {
            key: 'player-acid-idle-animation',
            frames: scene_game.anims.generateFrameNumbers('player-acid', { start: 0, end: 5, first: 5 }),
            frameRate: 4,
            repeat: -1
        };
        scene_game.anims.create(player_acid_idle_config);


        let player_acid_left_config = {
            key: 'player-acid-left-animation',
            frames: scene_game.anims.generateFrameNumbers('player-acid', { start: 6, end: 11, first: 11 }),
            frameRate: 4,
            repeat: -1
        };

        scene_game.anims.create(player_acid_left_config);

        let player_acid_right_config = {
            key: 'player-acid-right-animation',
            frames: scene_game.anims.generateFrameNumbers('player-acid', { start: 12, end: 17, first: 17 }),
            frameRate: 4,
            repeat: -1
        };
        scene_game.anims.create(player_acid_right_config);

        let player_acid_up_config = {
            key: 'player-acid-up-animation',
            frames: scene_game.anims.generateFrameNumbers('player-acid', { start: 18, end: 23, first: 23 }),
            frameRate: 4,
            repeat: -1
        };

        scene_game.anims.create(player_acid_up_config);

        let player_acid_down_config = {
            key: 'player-acid-down-animation',
            frames: scene_game.anims.generateFrameNumbers('player-acid', { start: 24, end: 29, first: 29 }),
            frameRate: 4,
            repeat: -1
        };
        scene_game.anims.create(player_acid_down_config);


    } 
    // DESTROYER
    else if(key === 'player-destroyer') {
        loaded_object_type_id = 271;
        loaded_type = "ship";


        let player_destroyer_left_config = {
            key: 'player-destroyer-left-animation',
            frames: scene_game.anims.generateFrameNumbers('player-destroyer', { start: 0, end: 2, first: 2 }),
            frameRate: 4,
            repeat: -1
        };

        scene_game.anims.create(player_destroyer_left_config);

        let player_destroyer_right_config = {
            key: 'player-destroyer-right-animation',
            frames: scene_game.anims.generateFrameNumbers('player-destroyer', { start: 3, end: 5, first: 5 }),
            frameRate: 4,
            repeat: -1
        };

        scene_game.anims.create(player_destroyer_right_config);

        let player_destroyer_up_config = {
            key: 'player-destroyer-up-animation',
            frames: scene_game.anims.generateFrameNumbers('player-destroyer', { start: 6, end: 8, first: 8 }),
            frameRate: 4,
            repeat: -1
        };

        scene_game.anims.create(player_destroyer_up_config);

        let player_destroyer_down_config = {
            key: 'player-destroyer-down-animation',
            frames: scene_game.anims.generateFrameNumbers('player-destroyer', { start: 9, end: 11, first: 11 }),
            frameRate: 4,
            repeat: -1
        };

        scene_game.anims.create(player_destroyer_down_config);
    }
    // GARDEN SHIP
    else if(key === 'player-garden-ship') {

        console.log("Creating garden ship animation");
        loaded_object_type_id = 345;
        loaded_type = "ship";

        let player_garden_ship_config = {
            key: 'player-garden-ship-animation',
            frames: scene_game.anims.generateFrameNumbers('player-garden-ship', { start: 0, end: 1, first: 1 }),
            frameRate: 2,
            repeat: -1
        };
        scene_game.anims.create(player_garden_ship_config);
    }
    // INFERNO 
    if(key === 'player-inferno') {

        console.log("Processing inferno body");
        loaded_object_type_id = 453;
        loaded_type = "body";


        let player_inferno_idle_config = {
            key: 'player-inferno-idle-animation',
            frames: scene_game.anims.generateFrameNumbers('player-inferno', { start: 0, end: 5, first: 5 }),
            frameRate: 4,
            repeat: -1
        };
        scene_game.anims.create(player_inferno_idle_config);


        let player_inferno_left_config = {
            key: 'player-inferno-left-animation',
            frames: scene_game.anims.generateFrameNumbers('player-inferno', { start: 6, end: 11, first: 11 }),
            frameRate: 4,
            repeat: -1
        };

        scene_game.anims.create(player_inferno_left_config);

        let player_inferno_right_config = {
            key: 'player-inferno-right-animation',
            frames: scene_game.anims.generateFrameNumbers('player-inferno', { start: 12, end: 17, first: 17 }),
            frameRate: 4,
            repeat: -1
        };
        scene_game.anims.create(player_inferno_right_config);

        let player_inferno_up_config = {
            key: 'player-inferno-up-animation',
            frames: scene_game.anims.generateFrameNumbers('player-inferno', { start: 18, end: 23, first: 23 }),
            frameRate: 4,
            repeat: -1
        };

        scene_game.anims.create(player_inferno_up_config);

        let player_inferno_down_config = {
            key: 'player-inferno-down-animation',
            frames: scene_game.anims.generateFrameNumbers('player-inferno', { start: 24, end: 29, first: 29 }),
            frameRate: 4,
            repeat: -1
        };
        scene_game.anims.create(player_inferno_down_config);


    } 
    // RESEARCHING
    else if(key === 'player-researching') {
...
function initiateMovePlayerFlow(player_index, destination_coord_type, destination_coord, source = '') {
    ...
    if (players[player_index].sprite) {
        ...
        /********** ACID *************/
        if (players[player_index].sprite.texture.key === 'player-acid') {

            // LEFT !
            if (players[player_index].destination_x < players[player_index].sprite.x) {

                if (players[player_index].sprite.anims.getCurrentKey() !== 'player-acid-left-animation') {
                    players[player_index].sprite.anims.play('player-acid-left-animation');
                }

            }
            // RIGHT!
            else if (players[player_index].destination_x > players[player_index].sprite.x) {
                if (players[player_index].sprite.anims.getCurrentKey() !== 'player-acid-right-animation') {
                    players[player_index].sprite.anims.play('player-acid-right-animation');
                }

            }
            // UP!
            else if (players[player_index].destination_y < players[player_index].sprite.y) {
                if (players[player_index].sprite.anims.getCurrentKey() !== 'player-acid-up-animation') {
                    players[player_index].sprite.anims.play('player-acid-up-animation');
                }
            }
            // DOWN!
            else if (players[player_index].destination_y > players[player_index].sprite.y) {
                if (players[player_index].sprite.anims.getCurrentKey() !== 'player-acid-down-animation') {
                    players[player_index].sprite.anims.play('player-acid-down-animation');
                }
            }
        }
        /***************** BLOCKADE RUNNER ******************/
        else if (players[player_index].sprite.texture.key === 'player-blockade-runner') {
        ...
        /********** INFERNO *************/
        else if (players[player_index].sprite.texture.key === 'player-inferno') {

            // LEFT !
            if (players[player_index].destination_x < players[player_index].sprite.x) {

                if (players[player_index].sprite.anims.getCurrentKey() !== 'player-inferno-left-animation') {
                    players[player_index].sprite.anims.play('player-inferno-left-animation');
                }

            }
            // RIGHT!
            else if (players[player_index].destination_x > players[player_index].sprite.x) {
                if (players[player_index].sprite.anims.getCurrentKey() !== 'player-inferno-right-animation') {
                    players[player_index].sprite.anims.play('player-inferno-right-animation');
                }

            }
            // UP!
            else if (players[player_index].destination_y < players[player_index].sprite.y) {
                if (players[player_index].sprite.anims.getCurrentKey() !== 'player-inferno-up-animation') {
                    players[player_index].sprite.anims.play('player-inferno-up-animation');
                }
            }
            // DOWN!
            else if (players[player_index].destination_y > players[player_index].sprite.y) {
                if (players[player_index].sprite.anims.getCurrentKey() !== 'player-inferno-down-animation') {
                    players[player_index].sprite.anims.play('player-inferno-down-animation');
                }
            }
        }
        /************ LANCER *******************/
...
function movePlayerInstant(player_index, x, y) {
    ...
    if (!players[player_index].sprite) {
        console.log("Returning false in movePlayerInstant. Still no sprite somehow");
        return false;
    }
...
function showClickMenu(pointer) {
    ...
    if (coord_index !== -1) {
        ...
        if (coord.player_id && coord.player_id !== client_player_id && current_view !== 'galaxy') {
            ...
            sel_click_menu.append("<br><a target='_blank' href='https://space.alphacoders.com/player/view/" + players[other_player_index].id + "'>View Profile</a>");
        }
    ...
    if (object_types[object_type_index].can_have_inventory) {
        ...
        player_inventory_items.forEach(function (inventory_item) {
            ...
            if (inventory_item_object_type_index !== -1) {
                ...
                /*
                sel_click_menu.append("<img style='width:28px;' src='https://space.alphacoders.com/" + urlName(object_types[object_type_index].name) + ".png'>" +
    ...
    if (object_types[object_type_index].can_be_attacked || (object_types[object_type_index].id === 114 && isFalse(objects[object_index].player_id)) ) {
        ...
        if (player_is_attacking) {
            ...
            if (objects[object_index].player_id && objects[object_index].player_id === client_player_id) {
                ...
                sel_click_menu.append("<br><button id='destroy_" + objects[object_index].id + "' object_id='" + objects[object_index].id +
            ...
            // Just an attack option
                ...
                if(!pod_in_galaxy) {
                    ...
                    sel_click_menu.append("<br><a target='_blank' href='https://space.alphacoders.com/site/tutorial/ship-building'>Build A Better Ship To Attack</a>");
                }
...
function updatePlayer(data, player_index) {
    ...
    if (current_view === 'planet') {
        ...
        console.log("We're on a planet and the other player's planet coord id: " + data.player.planet_coord_id);
        ...
        if (!data.player.planet_coord_id || data.player.planet_coord_id === 0) {
            ...
            if (coord_index !== -1 && shouldDraw(client_player_info.coord, planet_coords[coord_index])) {
                ...
                if (!players[player_index].sprite) {
                    console.log("no sprite. Lets set the planet coord id and create the player sprite!");
                    players[player_index].planet_coord_id = data.player.planet_coord_id;

                    // For something like a move from a portal, we'll still potentially have a ship coord id 
                    // set on the player right now, not yet updated from data.
                    // Our getPlayerInfo function checks for a ship_coord_id before it checks
                    // for a planet coord id
                    if(players[player_index].ship_coord_id) {
                    ...
                    console.log("After createPlayerSprite");
                    if(players[player_index].sprite) {
                        console.log("Player has a sprite now");
                    } else {
                        console.log("Still don't have player sprite");
                    }
                    ...
                    console.log("Should have moved player " + players[player_index].name + "to planet coord " + planet_coords[coord_index].id);
                }
                // Otherwise, flow them there
                else if (data.player.planet_coord_id !== players[player_index].planet_coord_id) {
                    console.log("We already have the sprite for this other player");
                    players[player_index].planet_coord_id = data.player.planet_coord_id;
                    updated_planet_coord_id = true;
        ...
        if (!data.player.ship_coord_id || data.player.ship_coord_id === 0) {
            ...
            // Our client doesn't have the coord where the player is, or the player moved somewhere out of drawing range
            else if (coord_index === -1 || !shouldDraw(client_player_info.coord, ship_coords[coord_index])) {
                destroyPlayerSprite(players[player_index]);
                players[player_index].destination_x = false;
                players[player_index].destination_y = false;
            }
...
function updatePlayerClient(data) {
    ...
    if (current_view === 'galaxy' && parseInt(data.player.ship_id) !== players[client_player_index].ship_id) {
        ...
        // and remove the new ship from the coords its on (we are using a sprite now)
        for(let i = 0; i < coords.length; i++) {
            if(coords[i] && ( coords[i].object_id === objects[new_ship_index].id || coords[i].belongs_to_object_id === objects[new_ship_index].id) ) {
                map.putTileAt(-1, coords[i].tile_x, coords[i].tile_y, false, 'layer_object');
            }
        }
