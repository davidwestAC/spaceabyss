...
$(document).on('click', 'button', async function () {
    ...
    if(split_name[0] == 'assemblefloor') {
        ...
        let clicked_tile_x = false;
        let clicked_tile_y = false;
        console.log("tile_x attribute: " + $('#' + clicked_id).attr('tile_x'));
        let being_assembled_in_object_id = $('#' + clicked_id).attr('being_assembled_in_object_id');
        if($('#' + clicked_id).attr('tile_x')) {
            console.log("Assembly click has tile_x");
            clicked_tile_x = $('#' + clicked_id).attr('tile_x');
            clicked_tile_y = $('#' + clicked_id).attr('tile_y');
    ...
    if(split_name[0] === 'help') {
        ...
        sel_click_menu.append("<a target='_blank' class='button is-success is-small' href='https://space.alphacoders.com/site/tutorial'>View Our Tutorial!</a><br>");
        if(client_player_index !== -1 && players[client_player_index]) {
            sel_click_menu.append("<a target='_blank' class='button is-info is-small' href='https://space.alphacoders.com/player/view/" + players[client_player_index].id + "'>Report A Problem</a><br>");
        } 
        ...
        for(let i = recent_failure_messages.length - 1; i > recent_failure_messages.length - 10; i--) {
            ...
            if(recent_failure_messages[i]) {
                ...
                if(recent_failure_messages[i].text === 'Your beam mangled up something advanced') {
                    sel_click_menu.append("<a target='_blank' class='button is-small' href='https://space.alphacoders.com/site/tutorial/salvaging'>Salvaging Tutorial</a> This object has some complex/advanced salvage opportunities. " + 
                ...
                if(recent_failure_messages[i].text === 'Ship Has No Floor Space') {
                    sel_click_menu.append("<a target='_blank' class='button is-small' href='https://space.alphacoders.com/site/tutorial/mining'>Mining Tutorial</a> By default, the things your ship " + 
                ...
                if(recent_failure_messages[i].text === 'Your beam is a bit weak') {
                    sel_click_menu.append("<a target='_blank' class='button is-small' href='https://space.alphacoders.com/site/tutorial/mining'>Mining Tutorial</a> This object has some complex/advanced mining opportunities. " + 
                ...
                if(recent_failure_messages[i].text.includes('Assembling') && recent_failure_messages[i].text.includes('Failed')) {
                    sel_click_menu.append("<a target='_blank' class='button is-small' href='https://space.alphacoders.com/site/tutorial/manufacturing'>Manufacturing Tutorial</a> The object that you " + 
    ...
    if(split_name[0] === 'showaioptions') {
        ...
        if(sel_ai_management.is(":visible")) {
            sel_ai_management.hide();
            sel_ai_management.empty();


        } else {
            sel_ai_management.show();
    ...
    if(split_name[0] === 'showareaoptions') {
        ...
        if(sel_area_management.is(":visible")) {
            sel_area_management.hide();
            sel_area_management.empty();


        } else {
            sel_area_management.show();
